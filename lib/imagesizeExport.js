...
ImageSizeExport.prototype.categorizeData = function (data) {
	...
	switch (this.options.categorizeBy) {
		...
		case 'folders':
			categorizedData = this.categorizeByFolders(data);
			break;
...
ImageSizeExport.prototype.categorizeByBreakpoints = function (data) {
	var bps = data.breakpoints.map(function (bp) {
		var breakpoint = {};
		breakpoint[bp] = [];
		return breakpoint;
	}, {});
	...
	data.images.forEach(function (image) {
		bps.forEach(function (bp) {
			if (bp[image.breakpoint] !== undefined) {
				bp[image.breakpoint].push(image);
			}
		});
	});
...
ImageSizeExport.prototype.categorizeByFolders = function (data) {
	var folders = data.folders.map(function (folder) {
		var folders = {};
		folders[folder] = [];
		return folders;
	}, {});
	...
	data.images.forEach(function (image) {
		folders.forEach(function (folder) {
			if (folder[image.folder] !== undefined) {
				folder[image.folder].push(image);
			}
		});
	});
